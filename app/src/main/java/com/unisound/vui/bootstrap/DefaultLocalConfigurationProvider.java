...
public class DefaultLocalConfigurationProvider {
    ...
    public DefaultLocalConfigurationProvider(Context context) {
    ...
    public void copyJsgfFromAssets() {
        ...
        if (!FileHelper.copyDirectoryFromAssets(this.context, JSGF_DIR, FileHelper.getJSGFDirectory(this.context))) {
            throw new RuntimeException("can't copy the jsgf from assets");
        }
        if (!FileHelper.getNluDirectory(this.context).exists() && !FileHelper.copyDirectoryFromAssets(this.context, NLU_DIR, FileHelper.getNluDirectory(this.context))) {
    ...
    public ANTELocalConfiguration getConfig() {
        ...
        for (int i = 0; i < length; i++) {
            ...
            String strSubstring = str.substring(0, str.lastIndexOf(NLU_NAME));
            ANTELocalConfiguration.LocalRecognition localRecognition = new ANTELocalConfiguration.LocalRecognition();
            localRecognition.setCompileJsgfPath(FileHelper.getJSGFFile(this.context, strSubstring + JSGF_NAME).getAbsolutePath());
            localRecognition.setGrammerPath(FileHelper.getGrammarFile(this.context, strSubstring + GRAMMAR_NAME).getAbsolutePath());
            localRecognition.setLocalNluPath(FileHelper.getNluFile(this.context, strSubstring + NLU_NAME).getAbsolutePath());
            aNTELocalConfiguration.addConfiguration(strSubstring, localRecognition);
        }
