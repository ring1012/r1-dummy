...
public class UniExoPlayer implements ExoPlayer.EventListener, MusicPlayer {
    ...
    private MediaSource buildMediaSource(Uri uri, String overrideExtension) {
        int iInferContentType = TextUtils.isEmpty(overrideExtension) ? Util.inferContentType(uri) : Util.inferContentType("." + overrideExtension);
    ...
    private void prepare(String url, boolean playWhenReady) {
        ...
        MediaSource mediaSourceBuildMediaSource = buildMediaSource(Uri.parse(url), null);
        this.player.seekTo(0L);
        this.player.prepare(mediaSourceBuildMediaSource);
    ...
    private void resetNotifyVolume() {
        if (this.isAutoSetAlarm) {
            ((AudioManager) this.context.getSystemService("audio")).setStreamVolume(3, this.currentNotifyVolume, 0);
            this.isAutoSetAlarm = false;
        }
    }
