...
public class c implements HttpStack {
    ...
    private static HttpEntity a(Response response) {
        ...
        basicHttpEntity.setContent(responseBodyBody.byteStream());
        basicHttpEntity.setContentLength(responseBodyBody.contentLength());
        basicHttpEntity.setContentEncoding(response.header("Content-Encoding"));
        if (responseBodyBody.contentType() != null) {
            basicHttpEntity.setContentType(responseBodyBody.contentType().type());
        }
    ...
    public HttpResponse performRequest(com.android.volley.Request<?> request, Map<String, String> additionalHeaders) throws IOException, AuthFailureError {
        ...
        OkHttpClient.Builder builderNewBuilder = this.f370a.newBuilder();
        builderNewBuilder.connectTimeout(timeoutMs, TimeUnit.MILLISECONDS).readTimeout(timeoutMs, TimeUnit.MILLISECONDS).writeTimeout(timeoutMs, TimeUnit.MILLISECONDS);
        ...
        a(builder, request);
        Response responseExecute = okHttpClientBuild.newCall(builder.build()).execute();
        BasicHttpResponse basicHttpResponse = new BasicHttpResponse(new BasicStatusLine(a(responseExecute.protocol()), responseExecute.code(), responseExecute.message()));
        basicHttpResponse.setEntity(a(responseExecute));
        Headers headers2 = responseExecute.headers();
        int size = headers2.size();
        for (int i = 0; i < size; i++) {
            String strName = headers2.name(i);
            String strValue = headers2.value(i);
            if (strName != null) {
                basicHttpResponse.addHeader(new BasicHeader(strName, strValue));
            }
        }
