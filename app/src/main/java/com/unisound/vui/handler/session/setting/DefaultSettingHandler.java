...
public class DefaultSettingHandler<I extends Intent> extends SimpleUserEventInboundHandler<NLU<Intent, Result.NullResult>> {
    ...
    private HashMap<String, Integer> volumeOperation = new HashMap<>();
    ...
    public void userEventTriggered(Object evt, ANTHandlerContext ctx) throws Exception {
        if ((evt instanceof Integer) && ((Integer) evt).intValue() == 3107) {
            String setWakeupFail = SpeakerTTSUtil.getTTSString(R.string.tts_wakeupword_set_fail, -1, ctx.androidContext());
            ctx.playTTS(setWakeupFail);
        }
    ...
    public boolean acceptInboundEvent0(NLU<Intent, Result.NullResult> evt) throws Exception {
        ...
        if (SName.SETTING_COMMON.equals(service)) {
            this.type = 1;
            return true;
        }
        if (SName.GLOBAL_CMD.equals(service)) {
            this.type = 2;
            return true;
        }
        if (SName.HEALTH_INFO.equals(service)) {
            this.type = 3;
            return true;
        }
        if (!SName.SETTING_AIR.equals(service)) {
            return false;
        }
        this.type = 4;
        return true;
    ...
    protected boolean onWakeupEventSetWakeupwordDone(ANTHandlerContext ctx) {
        ...
        if (this.eventReceived && (Operands.ATTR_WAKEUP_WORD.equals(this.tempSetOperands) || Operands.ATTR_NAME.equals(this.tempSetOperands))) {
            ...
            if (!TextUtils.isEmpty(this.tempMainWakeup)) {
                String mainWord = String.format(ctx.androidContext().getString(R.string.tts_wakeupword_set_success), this.tempMainWakeup);
                ctx.playTTS(mainWord);
                return true;
            }
        }
    ...
    @Override // com.unisound.vui.handler.SimpleUserEventInboundHandler
    public void eventReceived(NLU<Intent, Result.NullResult> nlu, ANTHandlerContext ctx) throws Exception {
        super.eventReceived((DefaultSettingHandler<I>) nlu, ctx);
        LogMgr.i(TAG, "eventReceived type = " + this.type + ",evt" + nlu.toString());
        String ansewr = nlu.getText();
        if (this.type == 1) {
            SettingIntent intent = getSettingIntent(nlu);
            ...
            if (SettingCommonIntent.Operands_Common.ATTR_VOLUME.equals(operands)) {
                ...
                if ("ACT_SET".equals(operator)) {
                    if (DATA_TYPE_PERCENT.equals(dataType) || DATA_TYPE_INT.equals(dataType)) {
                        float percent = Float.parseFloat(value) / 100.0f;
                        int maxVolume = this.volumeOperator.getMaxVolume();
                        int volume = Math.min(Math.max(1, Math.round(maxVolume * percent)), maxVolume);
                ...
                if (this.volumeOperation.containsKey(operator)) {
            ...
            } else if ("OBJ_LIGHT".equals(operands)) {
                handlePhicommSmartHomeProtocal(nlu, ansewr);
                return;
            }
        } else if (this.type == 0) {
            SettingIntent settingIntent = getSettingIntent(nlu);
            ...
            if (SCode.SETTING_EXEC.equals(nlu.getCode())) {
                ...
                if (Operator.ACT_CANCEL.equals(operate)) {
                    ...
                    if (Operands.ATTR_WAKEUP_WORD.equals(operands2) || Operands.ATTR_NAME.equals(operands2)) {
                        ...
                        if (isPhicommSmartHome(operands2, value2)) {
                            handlePhicommSmartHomeProtocal(nlu, ansewr);
                            return;
                        }
                        if (Operands.OBJ_BLUETOOTH.equals(operands2)) {
                            if ("ACT_OPEN".equals(operate)) {
                                this.mSettingHandler.handleOrder(Operator.ACT_PHICOMM_OPEN_BLUETOOTH, nlu);
                                return;
                            } else {
                                if ("ACT_CLOSE".equals(operate)) {
                                    this.mSettingHandler.handleOrder(Operator.ACT_PHICOMM_CLOSE_BLUETOOTH, nlu);
                                    return;
                                }
                                return;
                            }
                        }
                        if (OBJ_AMBIENTLIGHT.equals(operands2)) {
                            if ("ACT_OPEN".equals(operate)) {
                                this.mSettingHandler.handleOrder(ACT_PHICOMM_OPEN_AMBIENTLIGHT, nlu);
                                return;
                            } else {
                                if ("ACT_CLOSE".equals(operate)) {
                                    this.mSettingHandler.handleOrder(ACT_PHICOMM_CLOSE_AMBIENTLIGHT, nlu);
                                    return;
                                }
                                return;
                            }
                        }
                        if (OBJ_SOUNDEFFECT.equals(operands2)) {
                            if ("ACT_OPEN".equals(operate)) {
                                this.mSettingHandler.handleOrder(ACT_PHICOMM_OPEN_SOUNDEFFECT, nlu);
                                return;
                            } else {
                                if ("ACT_CLOSE".equals(operate)) {
                                    this.mSettingHandler.handleOrder(ACT_PHICOMM_CLOSE_SOUNDEFFECT, nlu);
                                    return;
                                }
                                return;
                            }
                        }
                        if (Operands.OBJ_SLEEP_MODE.equals(operands2) && "ACT_OPEN".equals(operate)) {
                            this.mSettingHandler.handleOrder(Operator.ACT_PHICOMM_OPEN_SLEEP_MODE, nlu);
                            return;
                        }
                        if (Operands.OBJ_TTS_MODLE.equals(operands2)) {
                            if (Operator.ACT_CHANGE.equals(operator2)) {
                                NLU nlu2 = new NLU();
                                nlu2.setService(SName.SHOW_SETTING_CENTER);
                                nlu2.setCode("SETTING_COMMAND_CHANGE_TTS_MODLE");
                                ctx.pipeline().fireUserEventTriggered(nlu2);
                            }
                            ...
                            if ("ACT_CLOSE".equals(operate)) {
                                ...
                                ansewr = nlu.getGeneral().getText();
                            }
                        } else {
                            if (("ACT_OPEN".equals(operate) || "ACT_CLOSE".equals(operate)) && !TextUtils.isEmpty(deviceType)) {
                                handlePhicommSmartHomeProtocal(nlu, ansewr);
                                return;
                            }
                            if (TextUtils.isEmpty(deviceType) && fireToMusic(asr, operator2)) {
                                exitAfterEnterOtherSession();
                                return;
                            } else if (isGlobalCancelword(nlu.getText())) {
                                this.isGlobalCancel = true;
                                ansewr = SpeakerTTSUtil.getTTSString(R.string.tts_cancel, -1, ctx.androidContext());
                            } else if (nlu.getGeneral() != null) {
                                ansewr = nlu.getGeneral().getText();
                            }
                        }
                    }
                ...
                if (SCode.SETTING_IOT.equals(nlu.getCode())) {
                    handlePhicommSmartHomeProtocal(nlu, ansewr);
                    return;
                }
                if (SCode.USER_COMMAND.equals(nlu.getCode())) {
                    String result = nlu.getText() != null ? nlu.getText().replace(PinyinConverter.PINYIN_SEPARATOR, "") : "";
                    CommandInfo info = LocalizeDataUtil.getCommandInfo(result);
                    if (info != null) {
                ...
                } else if (SCode.QUERY_SETTING.equals(nlu.getCode()) && Operands.ATTR_HOME_AIRQUALITY.equals(operands2)) {
                    handlePhicommSmartHomeProtocal(nlu, ansewr);
                    return;
                }
            }
        ...
        ctx.playTTS(ansewr);
        sendFullLogToDeviceCenter(nlu, ansewr);
    }
    ...
    private SettingIntent getSettingIntent(NLU<Intent, Result.NullResult> evt) {
        ...
        if (settingExtIntent == null || settingExtIntent.getOperations() == null || !settingExtIntent.getOperations().iterator().hasNext()) {
            SettingIntent settingIntent = (SettingIntent) evt.getSemantic().getIntent();
            return settingIntent;
        }
        SettingIntent settingIntent2 = settingExtIntent.getOperations().iterator().next();
        return settingIntent2;
    }
    ...
    /* JADX WARN: Multi-variable type inference failed */
    @Override // com.unisound.vui.handler.SimpleUserEventInboundHandler
    public String getInterruptType(NLU<Intent, Result.NullResult> nlu) {
        boolean isCancelOperate = isCancelOperate(nlu);
        LogMgr.d(TAG, "getInterruptType isCancelOperate:" + isCancelOperate);
        return isCancelOperate ? super.getInterruptType((DefaultSettingHandler<I>) nlu) : super.getInterruptType((DefaultSettingHandler<I>) nlu);
    }
    ...
    private boolean isCancelOperate(NLU nlu) {
        ...
        if (SName.GLOBAL_CMD.equals(serviceName)) {
            String operate = ((GlobalCmdIntent) nlu.getSemantic().getIntent()).getConfirm();
            if (Operator.ACT_CANCEL.equals(operate)) {
                return true;
            }
    ...
    private String checkWakeupAndAnswer(String wakeup) {
        if (TextUtils.isEmpty(wakeup)) {
            String answer = SpeakerTTSUtil.getTTSString(R.string.tts_wakeupword_unsupport, -1, this.ctx.androidContext());
            return answer;
        }
        ...
        if (!matchWakeUpWord(wordToCheck)) {
            String answer2 = SpeakerTTSUtil.getTTSString(R.string.tts_wakeupword_unsupport, -1, this.ctx.androidContext());
            return answer2;
        }
        ...
        if (wakeup2.equals(nickName) || UserPerferenceUtil.getWakeupWord(this.ctx.androidContext()).contains(wakeup2)) {
            String answer3 = SpeakerTTSUtil.getTTSString(R.string.tts_wakeupword_duplicate, -1, this.ctx.androidContext());
            return answer3;
        }
    ...
    public boolean matchWakeUpWord(String word) {
        Pattern p = Pattern.compile("^[\\u4e00-\\u9fa5]{2,3}$");
    ...
    private boolean isGlobalCancelword(String recTxt) {
        ...
        return recTxt.contains(byeString) || recTxt.contains(exitString);
    }
}
