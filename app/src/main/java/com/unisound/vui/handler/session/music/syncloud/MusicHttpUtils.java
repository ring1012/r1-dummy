...
public class MusicHttpUtils {
    ...
    public static void uploadMusicList(final String udid, final List<PlayItem> playItemList) {
        ThreadUtils.executeInSingle(new Runnable() { // from class: com.unisound.vui.handler.session.music.syncloud.MusicHttpUtils.1
            ...
            public void run() throws JSONException {
                ...
                RequestBody.ClientInfo clientInfo = new RequestBody.ClientInfo();
                body.setTcl(clientInfo);
                ...
                LogMgr.d(MusicHttpUtils.TAG, "uploadMusicList failure, " + (response == null ? "response is null!" : " response code = " + response.code()));
            }
        });
    }
}
