...
public class NaviConfig {
    ...
    public static Map<Object, Object> configs;
    ...
    public static void initMapOprsAndOprWpWords() {
        String[] stringArray;
        int iIntValue = ((Integer) getNaviConfig(MAPOPRSARRAYID)).intValue();
        if (context == null || (stringArray = context.getResources().getStringArray(iIntValue)) == null || stringArray.length <= 0) {
            return;
        }
        for (String str : stringArray) {
            if (str.contains(MqttTopic.MULTI_LEVEL_WILDCARD)) {
                String[] strArrSplit = str.split(MqttTopic.MULTI_LEVEL_WILDCARD);
                if (strArrSplit[0] != null && strArrSplit.length > 0) {
                    if (strArrSplit[0].contains(",")) {
                        String[] strArrSplit2 = strArrSplit[0].split(",");
                        for (int i = 0; i < strArrSplit2.length; i++) {
                            if (strArrSplit2[i] != null) {
                                naviSuportWakeUpWords.add(strArrSplit2[i].replaceAll("\\s*", ""));
                                if (strArrSplit[1] != null) {
                                    mapOprsToMethod.put(strArrSplit2[i], strArrSplit[1].replaceAll("\\s*", ""));
                                }
                                if (strArrSplit.length > 2) {
                                    mapOprsToUrl.put(strArrSplit2[i], strArrSplit[2].replaceAll("\\s*", ""));
                                }
                            }
                        }
                    } else {
                        naviSuportWakeUpWords.add(strArrSplit[0].replaceAll("\\s*", ""));
                        if (strArrSplit.length > 1) {
                            mapOprsToMethod.put(strArrSplit[0], strArrSplit[1].replaceAll("\\s*", ""));
                        }
                        if (strArrSplit.length > 2) {
                            mapOprsToUrl.put(strArrSplit[0], strArrSplit[2].replaceAll("\\s*", ""));
                        }
                    }
                }
            }
