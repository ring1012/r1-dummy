...
public class StringToPyUtil {
    ...
    private static char Char2Initial(char ch) {
        ...
        if ((ch >= 'A' && ch <= 'Z') || (iGbValue = gbValue(ch)) < BEGIN || iGbValue > END) {
            return ch;
        }
        int i = 0;
        while (i < 26 && (iGbValue < table[i] || iGbValue >= table[i + 1])) {
            i++;
        }
        if (iGbValue == END) {
            i = 25;
    ...
    private static int gbValue(char ch) throws UnsupportedEncodingException {
        try {
