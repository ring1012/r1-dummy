...
public class MqttTransportChannel extends BaseTransportChannel implements Handler.Callback {
    ...
    private Runnable mRunnable = new Runnable() { // from class: com.unisound.ant.device.mqtt.MqttTransportChannel.4
        @Override // java.lang.Runnable
        public void run() {
            MqttTransportChannel.this.openConectMqttTask();
        }
    };
    ...
    public void createChannel(Context context, OnMQTTStatusChangeListener phicommMQTTStatausChange) {
        this.phicommMQTTStatausChange = phicommMQTTStatausChange;
        if (mqttParam == null) {
    ...
    @Override // com.unisound.ant.device.mqtt.BaseTransportChannel
    public void createChannel(Context context, ChannelParams params, OnMQTTStatusChangeListener onMQTTStatusChangeListener) {
        if (params == null) {
            LogMgr.e(TAG, "createChannel param is null");
        } else {
            channelParams = params;
            createChannel(context, onMQTTStatusChangeListener);
        }
    }
    ...
    public void sendData(final Object data) {
        if (channelParams != null && mqttParam != null) {
            ...
            sendMessage(topic, content.getBytes(), new IMqttActionListener() { // from class: com.unisound.ant.device.mqtt.MqttTransportChannel.1
                ...
                public void onFailure(IMqttToken iMqttToken, Throwable throwable) {
                    ...
                    MqttTransportChannel.this.cacheFailedMessage(data);
                }
            });
            return;
        }
    ...
    private String getMessageType(Object data) {
        if (data instanceof String) {
            try {
                BaseRequest baseRequest = (BaseRequest) new Gson().fromJson((String) data, BaseRequest.class);
                return baseRequest.getMessageType();
            } catch (Exception e) {
    ...
    public boolean connect(MqttClientParam param) {
        if (this.client != null && this.client.isConnected()) {
            return true;
        }
        try {
            this.client = new MqttAsyncClient(param.getConnectUrl(), param.getClientid(), null);
            ...
            this.client.setCallback(new MqttCall());
            try {
                LogMgr.d(TAG, "mqtt connecting, params = " + param.toString());
                IMqttToken mqttToken = this.client.connect(options);
    ...
    private void startConnect() {
        this.taskHandler.post(new Runnable() { // from class: com.unisound.ant.device.mqtt.MqttTransportChannel.2
            @Override // java.lang.Runnable
            public void run() {
                boolean isSuccess = MqttTransportChannel.this.connect(MqttTransportChannel.mqttParam);
                if (isSuccess) {
                    ...
                    MqttTransportChannel.this.onConnectFailure("mc_9999");
                }
            }
        });
    }
    ...
    private void reconnect() {
        ...
        this.taskHandler.post(new Runnable() { // from class: com.unisound.ant.device.mqtt.MqttTransportChannel.3
            @Override // java.lang.Runnable
            public void run() throws InterruptedException {
                Log.d(MqttTransportChannel.TAG, "reconnect mqtt, isNetworkConnected = " + NetUtil.isNetworkConnected(MqttTransportChannel.this.context));
                while (NetUtil.isNetworkConnected(MqttTransportChannel.this.context) && !MqttTransportChannel.this.mConnecting) {
                    MqttTransportChannel.this.mConnecting = true;
                    boolean isSuccess = MqttTransportChannel.this.connect(MqttTransportChannel.mqttParam);
                    LogMgr.d(MqttTransportChannel.TAG, "reconnect isSuccess:" + isSuccess);
                    if (!isSuccess) {
                        MqttTransportChannel.this.mConnecting = false;
                        try {
    ...
    private IMqttToken sendMessage(String topic, byte[] message, IMqttActionListener listener) {
        ...
        if (this.client != null && this.client.isConnected()) {
            return this.client.publish(topic, message, 1, false, null, listener);
        }
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public void cacheFailedMessage(Object data) {
        LogMgr.d(TAG, "cacheFailedMessage");
        String type = getMessageType(data);
        if ("synInfo".equals(type)) {
    ...
    private class MqttCall implements MqttCallback {
        private MqttCall() {
        }
        ...
        public void connectionLost(Throwable cause) {
            ...
            UserPerferenceUtil.setDeviceOnlineState(MqttTransportChannel.this.context, false);
        ...
        public void messageArrived(String topic, MqttMessage message) {
            ...
            content.what = 5;
            content.obj = msg;
            MqttTransportChannel.this.connectHandler.sendMessage(content);
        }
    ...
    private class ServerTask extends AsyncTask<Void, Void, Void> {
        ...
        public Void doInBackground(Void... params) {
            MqttTransportChannel.this.registerMqtt();
            return null;
        }
        ...
        public void onPostExecute(Void result) {
            super.onPostExecute((ServerTask) result);
        }
    }
    ...
    public void registerMqtt() {
        ...
        if (result != null && result.getResult() != null) {
            ...
            this.connectHandler.postDelayed(this.mRunnable, 300L);
        }
    }
    ...
    private void sendOnlineMessage(boolean isOnline) {
        ...
        info.setPassportId(0L);
        info.setSubsystemId(4);
        info.setExtras("");
        message.setClientInfo(info);
        String onlineMsg = new Gson().toJson(message);
        sendData(onlineMsg);
    }
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public void openConectMqttTask() {
        this.registerTimes++;
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public void onConnectFailure(String regitstMqttErrorCode) {
        this.mConnecting = false;
        this.phicommMQTTStatausChange.onFail(regitstMqttErrorCode);
    }
    ...
    private void connectPrepared(com.unisound.ant.device.mqtt.bean.MqttMessage result) {
        ...
        String connectUrl = setEnvironmentParam(connectionInfo);
        mqttParam.setConnectUrl(connectUrl);
    ...
    private String setEnvironmentParam(MqttConnection connectionInfo) {
        if (ANTConfigPreference.isDev()) {
            String connectUrl = connectionInfo.getProtocol() + INNER_ADDR;
            return connectUrl;
        }
        String connectUrl2 = connectionInfo.getProtocol() + "://" + connectionInfo.getIp() + ":" + connectionInfo.getPort();
        return connectUrl2;
    }
}
