...
public class DefaultVolumeOperator implements VolumeOperator {
    ...
    private CopyOnWriteArrayList<VolumeListener> mVolumeListeners = new CopyOnWriteArrayList<>();
    ...
    public void setVolumeRaise() {
        int volume = getMusicCurrentVolume();
        setVolume(volume + 1);
    }
    ...
    public void setVoiceVolume(float percent) {
        ...
        setVolume(Math.round(getMusicMaxVolume() * percent));
    }
    ...
    private void fireVolumeChange() {
        ...
        while (volumeListenerIterator.hasNext()) {
            volumeListenerIterator.next().onVolumeChanged(Integer.valueOf(currentVolume), Integer.valueOf(maxVolume), Float.valueOf(currentVolume / maxVolume));
        }
    }
}
