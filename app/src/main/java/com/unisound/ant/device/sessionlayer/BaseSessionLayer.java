...
public final class BaseSessionLayer implements UpDownMessageManager.MessageExcuteListener {
    ...
    public BaseSessionLayer(Context context, SessionUpdateCallBack updateCallBack) {
        this.upDownMessageManager = new UpDownMessageManager(context);
        this.upDownMessageManager.setMessageExcuteListener(this);
        this.updateCallBack = updateCallBack;
    }

    public void saveDialogStatus(String action, DialogProfile dialogProfile) {
        actionDialogs.put(action, dialogProfile);
    ...
    public void filterSessionContent(String message) {
        ...
        if (!TextUtils.isEmpty(message)) {
            Type classType = new TypeToken<CloudResponse<SessionData<JsonObject>>>() { // from class: com.unisound.ant.device.sessionlayer.BaseSessionLayer.1
            }.getType();
            ...
            if (CloudResponse.MESSAGE_TYPE_INTENT_ACTION.equals(messageType) || "synInfo".equals(messageType)) {
                try {
            ...
            if (CloudResponse.MESSAGE_TYPE_UPDATE_REPONSE.equals(messageType) || CloudResponse.MESSAGE_TYPE_UPLOAD_PD_REPONSE.equals(messageType)) {
