...
public class MqttMsgHelper {
    ...
    static {
        ...
        WEB_URL = ANTConfigPreference.getMsgCenterUrl();
    ...
    public static MqttMessage getUnreadMessages(MessageParam param) throws Exception {
        ...
        if (!TextUtils.isEmpty(result)) {
            MqttMessage message2 = (MqttMessage) new Gson().fromJson(result, MqttMessage.class);
            return message2;
        }
        return message;
    }
    ...
    public static String checkClientAuth(String clientid, String userName, String password) throws Exception {
        String url = WEB_URL + "/rest/v1/client/auth";
        String param = "clientid=" + clientid + "&username=" + userName + "&password=" + password;
        try {
