...
import com.unisound.ant.platform.smarthome.SmartExecutor;
...
public class SmartDeviceMgr extends SessionExecuteHandler implements SmartExecutor.ExecutorCallBack {
    ...
    public void handleMessage(Message msg) {
        switch (msg.what) {
            case 0:
                ...
                dispatchSmartHouseControlCommand(command);
                break;
        }
    }
    ...
    private void dispatchSmartHouseControlCommand(UnisoundDeviceCommand command) {
        ...
        JsonArray operations = parameter.getAsJsonArray("operations");
        Type type = new TypeToken<List<SmartAction>>() { // from class: com.unisound.ant.platform.smarthome.SmartDeviceMgr.1
        }.getType();
        List<SmartAction> actions = (List) JsonTool.fromJson(operations, type);
        if (actions == null) {
    ...
    public class ReportHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case 1:
                    SmartDeviceStatus status = (SmartDeviceStatus) msg.obj;
                    SmartDeviceMgr.this.updateExecuteStatus(DstServiceState.SERVICE_STATE_SETTING_OVER, CommandOperate.COMMAND_OPERATE_SMART_STATUS_SYNC, status);
                    break;
            }
        }
    }
    ...
    public void updateExecuteStatus(String dstStatus, String operate, SmartDeviceStatus aSmartStatus) {
