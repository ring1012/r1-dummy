...
public class PhicommStatusHandler extends ANTOutboundHandlerAdapter {
    ...
    public void write(ANTHandlerContext ctx, Object msg) throws Exception {
        if (msg instanceof MusicOutputEvents) {
            MusicOutputEvents outputEvents = (MusicOutputEvents) msg;
            if (outputEvents.getData().booleanValue()) {
                onStartMusic();
            ...
            DormantOutputEvent outputEvents2 = (DormantOutputEvent) msg;
            if (outputEvents2.getData().booleanValue()) {
                onStartDormant();
            } else {
                onStopDormant();
            }
        } else if (msg instanceof BluetoothOutputEvent) {
            BluetoothOutputEvent outputEvents3 = (BluetoothOutputEvent) msg;
            if (outputEvents3.getData().booleanValue()) {
                onStartBluetooth();
            } else {
                onStopBluetooth();
            }
        } else if (msg instanceof NetworkConfigOutputEvent) {
            NetworkConfigOutputEvent outputEvents4 = (NetworkConfigOutputEvent) msg;
            if (outputEvents4.getData().booleanValue()) {
                onStartNetConfig();
            } else {
                onStopNetConfig();
            }
        } else if (msg instanceof DormantMessageEvent) {
            DormantMessageEvent outputEvents5 = (DormantMessageEvent) msg;
            if (outputEvents5.getData().booleanValue()) {
