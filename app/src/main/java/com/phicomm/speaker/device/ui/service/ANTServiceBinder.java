...
public class ANTServiceBinder extends Binder {
    ...
    public ANTServiceBinder(Application application, ANTAudioSourceImpl antAudioSource) {
        BasicCredentials credentials = new BasicCredentials(ExoConstants.APP_KEY, ExoConstants.APP_SECRET);
        this.antEngineANTEFactory = new NativeANTEFactory(credentials, application, antAudioSource);
    ...
    private void initMode(final Application application) {
        ...
        Observable.create(new Observable.OnSubscribe<NativeBootstrap>() { // from class: com.phicomm.speaker.device.ui.service.ANTServiceBinder.2
        ...
        }).subscribeOn(Schedulers.newThread()).observeOn(AndroidSchedulers.mainThread()).subscribe(new Action1<NativeBootstrap>() { // from class: com.phicomm.speaker.device.ui.service.ANTServiceBinder.1
            @Override // rx.functions.Action1
            public void call(NativeBootstrap nativeBootstrap) {
                LogMgr.e("MainActivity", "ANTServiceBinder");
                nativeBootstrap.init();
                ANTServiceBinder.this.engineInited = true;
            }
        });
    }
