...
public final class ExampleANTEngineInitializer extends ANTEngineInitializer {
    ...
    protected void onEngineInitDone(ANTEngine engine) {
        ...
        String udid = (String) engine.config().getOption(ANTEngineOption.GENERAL_UDID);
        PhicommPlayer phicommPlayer = new PhicommPlayer(context, udid, engine);
        ...
        CustomRingingProcessor customRingingProcessor = new CustomRingingProcessor(ringingHandler);
        customApiManager.addListener(0, customRingingProcessor);
    }
}
