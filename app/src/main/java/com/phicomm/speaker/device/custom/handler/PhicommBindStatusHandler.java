...
public class PhicommBindStatusHandler extends ANTOutboundHandlerAdapter {
    ...
    public void write(ANTHandlerContext ctx, Object msg) throws Exception {
        if (msg instanceof BindStatusEvent) {
            BindStatusEvent event = (BindStatusEvent) msg;
    ...
    public void enterASR(ANTHandlerContext ctx) throws Exception {
        ThreadUtils.execute(new Runnable() { // from class: com.phicomm.speaker.device.custom.handler.PhicommBindStatusHandler.1
    ...
    private void clearWakeupWord() {
        ...
        List<String> wakeupWordList = Arrays.asList(this.mContext.getResources().getStringArray(R.array.default_wakeup_words));
        this.mANTEngine.updateWakeupWord(wakeupWordList);
    }

    /* JADX INFO: Access modifiers changed from: private */
    public void queryDeviceBoundStatus() {
        LogMgr.d(TAG, "query device bound status");
        String udid = (String) this.mANTEngine.config().getOption(ANTEngineOption.GENERAL_UDID);
        try {
