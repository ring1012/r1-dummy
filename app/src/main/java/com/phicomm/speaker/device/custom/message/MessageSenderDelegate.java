...
public class MessageSenderDelegate implements MessageDispatchManager.MessageReceiver {
    ...
    public void send(boolean loopUntilReply, int replyType, int subReplyType, int type, int subType, int msgId, Parcelable data) {
        if (loopUntilReply) {
            ...
            Message message = this.mSendHandler.obtainMessage(what, msg);
            this.mSendHandler.sendMessageDelayed(message, 3000L);
        }
    ...
    private class LoopSendHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            MessageSenderDelegate.this.send(true, phicommMsg.getReplyType(), phicommMsg.getSubReplyType(), phicommMsg.getType(), phicommMsg.getSubType(), phicommMsg.getMsgId(), phicommMsg.getData());
        }
    }
}
