...
public class PhicommInitializeHandler extends ANTEventDispatcher implements PhicommDeviceStatusProcessor.OnDeviceStatusChangedListener {
    ...
    protected boolean onASREventEngineInitDone(ANTHandlerContext ctx) throws IllegalStateException, Resources.NotFoundException, IOException {
    ...
    private void playInitDoneTips() throws IllegalStateException, Resources.NotFoundException, IOException {
        try {
            this.mSysPrivateManager = (SysPrivateManager) this.mContext.getSystemService(FX_SYSTEM_PRIVATE);
            if (this.mSysPrivateManager != null) {
                String value = this.mSysPrivateManager.getBootProp(FX_KEY_OTA_MODE);
                if (FX_OTA_MODE_VALUE_SILENT.equals(value)) {
        ...
        if (!NetUtil.isNetworkConnected(this.mContext)) {
            UniMediaPlayer.getInstance().playBeepSound("", R.raw.bootloader_completed, new IMediaPlayerStateListener() { // from class: com.phicomm.speaker.device.custom.handler.PhicommInitializeHandler.1
    ...
    private void onDormantStatusChanged(boolean isDormant) {
        ...
        SelfDefinationResponseInfo selfDefinationResponseInfo = new SelfDefinationResponseInfo(SelfDefinationRequestInfo.MODIFY_DORMANT_STATUS, 0, content);
