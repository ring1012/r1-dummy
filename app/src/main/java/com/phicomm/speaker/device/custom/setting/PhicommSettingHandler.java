...
public class PhicommSettingHandler implements SettingHandler {
    ...
    public void handleOrder(String type) {
        switch (type) {
            case "ACT_PHICOMM_VERSION":
                sendPhicommVersion();
                break;
            case "ACT_PHICOMM_OTA":
                sendPhicommOTA();
                break;
            case "ACT_PHICOMM_BACKUP":
                sendphicommBackup();
                break;
            default:
    ...
    public void handleOrder(String type, Object data) {
        switch (type) {
            case "ACT_PHICOMM_SMART_HOME":
                sendPhicommSmartHomeCommand(data);
                break;
            case "ACT_PHICOMM_OPEN_AMBIENTLIGHT":
                openAmbientLight((NLU) data);
                break;
            case "ACT_PHICOMM_CLOSE_AMBIENTLIGHT":
                closeAmbientLight((NLU) data);
                break;
            case "ACT_PHICOMM_OPEN_SOUNDEFFECT":
                openSoundEffect((NLU) data);
                break;
            case "ACT_PHICOMM_CLOSE_SOUNDEFFECT":
                closeSoundEffect((NLU) data);
                break;
            case "ACT_PHICOMM_OPEN_BLUETOOTH":
                openBluetoothMode((NLU) data);
                break;
            case "ACT_PHICOMM_CLOSE_BLUETOOTH":
                closeBluetoothMode((NLU) data);
                break;
            case "ACT_PHICOMM_OPEN_SLEEP_MODE":
    ...
    private void onAmbientLightStatusChanged(boolean isAmbientLightOn) {
        ...
        SelfDefinationResponseInfo selfDefinationResponseInfo = new SelfDefinationResponseInfo(SelfDefinationRequestInfo.MODIFY_AMBIENT_LIGHT_STATUS, 0, content);
    ...
    private void sendPhicommSmartHomeCommand(Object nluData) {
        ...
        this.mHandler.postDelayed(ioTRunnable, 15000L);
    }

    private class PhicommIotReceiver implements MessageDispatchManager.MessageReceiver {
        ...
        public void notifyMsg(int what, int domain, int subdomain, Object data) {
            ...
            try {
                String dataJson = (String) ((ParcelableUtil) data).getValue();
                PhicommIotResponse result = (PhicommIotResponse) JsonTool.fromJson(dataJson, PhicommIotResponse.class);
    ...
    public void unregistPhicommMessageReceiver(PhicommIotReceiver receiver) {
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public void sendFullLogToDeviceCenter(NLU nlu, String ttsData) {
