...
public class PhicommMusicHandler extends DefaultMusicHandler implements MusicStatusListener {
    ...
    @Override // com.unisound.vui.handler.session.music.DefaultMusicHandler, com.unisound.vui.handler.SimpleUserEventInboundHandler, com.unisound.vui.engine.ANTDuplexHandler, com.unisound.vui.handler.ANTEventDispatcher
    ...
    @Override // com.unisound.vui.handler.session.music.DefaultMusicHandler, android.content.ServiceConnection
    ...
    public void onMusicStatusChanged(int status) {
        ...
        if (status == 2) {
            ...
            this.mHandler.postDelayed(this.mRecoveryWakeupWordRunnable, 1800000L);
            return;
        }
        if (status == 3 && this.mRecoveryWakeupWordRunnable != null) {
    ...
    private float getEffectBenchmark(int status) {
        return status == 3 ? getCurrentMusicWakeupBenchmark() : ANTConfigPreference.wakeupBenchmark;
    }
