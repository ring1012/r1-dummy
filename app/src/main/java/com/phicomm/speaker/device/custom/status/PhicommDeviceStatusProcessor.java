...
public class PhicommDeviceStatusProcessor implements IpcReceiver, SceneStateMachine.StatusChangedListener {
    ...
    public void onReceive(int domain, int subDomain, int sessionId, Object data) {
        if (subDomain == 51) {
            boolean isMusicOn = ((Boolean) ((ParcelableUtil) data).getValue()).booleanValue();
            if (isMusicOn) {
        ...
        if (subDomain == 52) {
            boolean isBtOn = ((Boolean) ((ParcelableUtil) data).getValue()).booleanValue();
            if (isBtOn) {
        ...
        if (subDomain == 53) {
            boolean isNetConfigOn = ((Boolean) ((ParcelableUtil) data).getValue()).booleanValue();
            if (isNetConfigOn) {
