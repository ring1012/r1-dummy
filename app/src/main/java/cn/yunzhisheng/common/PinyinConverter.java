...
public class PinyinConverter {
    ...
    private static String filterChar(String result) {
        return (result == null || result.equals("")) ? "" : result.replaceAll("( )+", PINYIN_SEPARATOR).replaceAll("[ ]*[|][ ]*", PINYIN_EXCLUDE).trim();
    }

    public static void getCartesianPinyin(int index, ArrayList<String[]> table, String str, ArrayList<String> names) {
        int i = 0;
        if (index == table.size() - 1) {
            String[] strArr = table.get(index);
            while (i < strArr.length) {
                names.add((String.valueOf(str) + PINYIN_SEPARATOR + strArr[i]).trim());
                i++;
            }
            return;
        }
        String[] strArr2 = table.get(index);
        while (i < strArr2.length) {
            getCartesianPinyin(index + 1, table, String.valueOf(str) + PINYIN_SEPARATOR + strArr2[i], names);
            i++;
        }
    }

    public static void getCartesianPinyin(int index, String[][] table, String str, ArrayList<String> names) {
        int i = 0;
        if (index == table.length - 1) {
            String[] strArr = table[index];
            while (i < strArr.length) {
                names.add(String.valueOf(str) + PINYIN_SEPARATOR + strArr[i]);
                i++;
            }
            return;
        }
        String[] strArr2 = table[index];
        while (i < strArr2.length) {
            getCartesianPinyin(index + 1, table, String.valueOf(str) + PINYIN_SEPARATOR + strArr2[i], names);
            i++;
        }
    ...
    private static int getIntFrombytes(byte[] b, int start, int len) {
        int i = 0;
        if (b == null || b.length < start + len) {
            return -1;
        }
        int i2 = 0;
        int i3 = 0;
        while (i2 < len) {
            int i4 = (b[start + i2] << (i2 * 8)) | (i & i3);
            i3 |= 255 << (i2 * 8);
            i2++;
            i = i4;
        }
    ...
    private static ArrayList<String> getMultiPinYin(String str) {
        ...
        for (int i = 0; i < str2.length(); i++) {
            arrayList.add(transWord2MultiPinyin(Character.valueOf(str2.charAt(i))).trim().split(PINYIN_SEPARATOR));
        }
        ArrayList<String> arrayList2 = new ArrayList<>();
        getCartesianPinyin(0, (ArrayList<String[]>) arrayList, "", arrayList2);
        return arrayList2;
    }

    public static String[] getMultiSpell(String cnStr, int type) {
        ArrayList<String> nameMultiPinYin;
        ...
        switch (type) {
            case 1:
                nameMultiPinYin = getNameMultiPinYin(cnStr);
                break;
            default:
                nameMultiPinYin = getMultiPinYin(cnStr);
                break;
        }
        if (nameMultiPinYin == null || nameMultiPinYin.size() <= 0) {
            return null;
        }
        String[] strArr = new String[nameMultiPinYin.size()];
        int i = 0;
        while (true) {
            int i2 = i;
            if (i2 >= nameMultiPinYin.size()) {
                return strArr;
            }
            strArr[i2] = nameMultiPinYin.get(i2);
            i = i2 + 1;
        }
    }
    ...
    private static ArrayList<String> getNameMultiPinYin(String str) {
        ...
        int i = 0;
        while (i < str2.length()) {
            Character chValueOf = Character.valueOf(str2.charAt(i));
            arrayList.add((i == 0 && mSpecialXingMap.containsKey(chValueOf)) ? new String[]{mSpecialXingMap.get(chValueOf)} : transWord2MultiPinyin(chValueOf).trim().split(PINYIN_SEPARATOR));
            i++;
        }
        ArrayList<String> arrayList2 = new ArrayList<>();
        getCartesianPinyin(0, (ArrayList<String[]>) arrayList, "", arrayList2);
    ...
    public static String[] getNameMultiSpell(String cnStr) {
        return getMultiSpell(cnStr, 0);
    }
    ...
    private static String getNamePinYin(String str) {
        ...
        boolean z2 = true;
        while (i < str.length()) {
            Character chValueOf = Character.valueOf(str.charAt(i));
            if (chValueOf == null || chValueOf.charValue() != PINYIN_SEPARATOR.charAt(0)) {
                String strTransWord2Pinyin = transWord2Pinyin(chValueOf);
                if (strTransWord2Pinyin != null && !strTransWord2Pinyin.equals("")) {
                    sb.append(PINYIN_SEPARATOR);
                    if (z2 && mSpecialXingMap.containsKey(chValueOf)) {
                        sb.append(mSpecialXingMap.get(chValueOf));
                        z2 = false;
                    } else {
                        sb.append(strTransWord2Pinyin);
                    }
                    sb.append(PINYIN_SEPARATOR);
                }
                if ((chValueOf.charValue() >= '0' && chValueOf.charValue() <= '9') || ((chValueOf.charValue() >= 'a' && chValueOf.charValue() <= 'z') || (chValueOf.charValue() >= 'A' && chValueOf.charValue() <= 'Z'))) {
                    sb.append(chValueOf);
                    z = z2;
                } else if (chValueOf.charValue() == PINYIN_EXCLUDE.charAt(0)) {
                    sb.append(chValueOf);
                    z = true;
                } else {
                    z = z2;
    ...
    public static String getNameSpell(String cnStr) {
        return (cnStr == null || cnStr.equals("")) ? "" : getSpell2(cnStr, 1)[0];
    }

    public static String[] getNameSpell1(String cnStr) {
        return getSpell(cnStr, 1);
    }

    public static String[] getNameSpell2(String cnStr) {
        return getSpell2(cnStr, 1);
    }
    ...
    private static String getPinYin(String str) {
        ...
        for (int i = 0; i < str.length(); i++) {
            ...
            if (chValueOf == null || chValueOf.charValue() != PINYIN_SEPARATOR.charAt(0)) {
                ...
                if ((chValueOf.charValue() >= '0' && chValueOf.charValue() <= '9') || ((chValueOf.charValue() >= 'a' && chValueOf.charValue() <= 'z') || ((chValueOf.charValue() >= 'A' && chValueOf.charValue() <= 'Z') || chValueOf.charValue() == PINYIN_EXCLUDE.charAt(0)))) {
                    sb.append(chValueOf);
                }
            } else {
                sb.append(chValueOf);
    ...
    private static String getPinYinbyIndex(int index) {
        if (mPinyinList != null) {
            int index2 = index * mPinyinSize;
            if (mPinyinSize + index2 <= mPinyinList.length) {
                StringBuilder sb = new StringBuilder();
                for (int i = index2; i < mPinyinSize + index2 && mPinyinList[i] != 0; i++) {
                    sb.append((char) mPinyinList[i]);
                }
                return sb.toString();
            }
        }
    ...
    public static String[] getSpell(String cnStr, int type) {
        String[] spell2 = getSpell2(cnStr, type);
        if (spell2 == null) {
            return null;
        }
    ...
    public static String[] getSpell2(String cnStr, int type) {
        String namePinYin;
        if (cnStr == null) {
            return null;
        }
        switch (type) {
            case 1:
                namePinYin = getNamePinYin(cnStr);
                break;
            default:
                namePinYin = getPinYin(cnStr);
                break;
        }
        StringBuffer stringBuffer = new StringBuffer();
        boolean z = true;
        for (int i = 0; i < namePinYin.length(); i++) {
            char cCharAt = namePinYin.charAt(i);
            if (z && ((cCharAt >= 'a' && cCharAt <= 'z') || (cCharAt >= 'A' && cCharAt <= 'Z'))) {
                stringBuffer.append(cCharAt);
            }
            if (cCharAt == PINYIN_SEPARATOR.charAt(0)) {
                z = true;
            } else if (cCharAt == PINYIN_EXCLUDE.charAt(0)) {
                stringBuffer.append(cCharAt);
                z = true;
            } else {
                z = false;
        ...
        return new String[]{namePinYin, stringBuffer.toString()};
    }

    private static int getUIntFrombytes(byte[] b, int start, int len) {
        int i = 0;
        if (b == null || b.length < start + len) {
            return -1;
        }
    ...
    public static String[] getWordMultiSpell(String cnStr) {
        return getMultiSpell(cnStr, 1);
    }

    public static String getWordSpell(String cnStr) {
        return getSpell2(cnStr, 2)[0];
    }

    public static String[] getWordSpell1(String cnStr) {
        return getSpell(cnStr, 2);
    }

    public static String[] getWordSpell2(String cnStr) {
        return getSpell2(cnStr, 2);
    }

    public static void init(InputStream in) throws IOException {
        if (mUnit2PinyinIndex == null) {
            try {
                if (in == null) {
                    return;
                }
                try {
                    ...
                    mPinyinSize = getUIntFrombytes(bArr, 4, 4);
                    int i = mPinyinNum * mPinyinSize;
                    if (i > 0) {
                        mPinyinList = new byte[i];
                        System.arraycopy(bArr, 8, mPinyinList, 0, i);
                    }
                    int i2 = i + 8;
                    int length = bArr.length - i2;
                    if (length > 0) {
                        mUnit2PinyinIndex = new byte[length];
                        System.arraycopy(bArr, i2, mUnit2PinyinIndex, 0, length);
                    }
                    try {
                        in.close();
    ...
    public static char transT2S(char t) {
        int iIndexOf = mTraditional.indexOf(t);
        return iIndexOf >= 0 ? mSimplified.charAt(iIndexOf) : t;
    }

    private static String transWord2MultiPinyin(Character cn2) {
        return transWord2Pinyin(cn2, true);
    }

    private static String transWord2Pinyin(Character cn2) {
        return transWord2Pinyin(cn2, false);
    }

    /* JADX WARN: Removed duplicated region for block: B:44:0x00be A[PHI: r1
  0x00be: PHI (r1v1 java.lang.String) = 
 ...
 binds: [B:4:0x001d, B:6:0x0020, B:8:0x0029, B:10:0x002e, B:12:0x0036, B:14:0x003d, B:19:0x004f] A[DONT_GENERATE, DONT_INLINE]] */
    /*
        Code decompiled incorrectly, please refer to instructions dump.
        To view partially-correct code enable 'Show inconsistent code' option in preferences
    */
    private static java.lang.String transWord2Pinyin(java.lang.Character r10, boolean r11) throws java.io.UnsupportedEncodingException {
        /*
            r3 = 0
            r2 = 0
            r8 = 2
            java.lang.String r1 = ""
            java.lang.String r0 = ""
            char r4 = r10.charValue()
            char r4 = transT2S(r4)
            java.lang.Character r10 = java.lang.Character.valueOf(r4)
            java.lang.String r4 = java.lang.String.valueOf(r10)     // Catch: java.io.UnsupportedEncodingException -> L90
            java.lang.String r5 = "UTF-16LE"
            byte[] r4 = r4.getBytes(r5)     // Catch: java.io.UnsupportedEncodingException -> L90
        L1d:
            if (r4 == 0) goto Lbe
            int r5 = r4.length
            if (r5 <= 0) goto Lbe
            int r5 = r4.length
            int r4 = getUIntFrombytes(r4, r2, r5)
            ...
            int r4 = r4 + (-19968)
            int r4 = r4 * 2
            byte[] r5 = cn.yunzhisheng.common.PinyinConverter.mUnit2PinyinIndex
            if (r5 == 0) goto Lbe
            byte[] r5 = cn.yunzhisheng.common.PinyinConverter.mUnit2PinyinIndex
            int r5 = r5.length
            int r6 = r4 + 2
            if (r5 <= r6) goto Lbe
            byte[] r5 = cn.yunzhisheng.common.PinyinConverter.mUnit2PinyinIndex
            int r4 = getIntFrombytes(r5, r4, r8)
            r5 = 9999(0x270f, float:1.4012E-41)
            if (r4 == r5) goto L4f
            if (r4 < 0) goto L4f
            java.lang.String r1 = getPinYinbyIndex(r4)
        L4f:
            if (r4 >= 0) goto Lbe
            int r0 = 20902 - r4
            int r0 = r0 * 2
            byte[] r1 = cn.yunzhisheng.common.PinyinConverter.mUnit2PinyinIndex
            int r4 = getIntFrombytes(r1, r0, r8)
            int r0 = r0 + 2
            int r1 = -r4
            int[] r5 = new int[r1]
        ...
        L6f:
            java.util.Map<java.lang.Character, java.lang.String> r2 = cn.yunzhisheng.common.PinyinConverter.mUncommonWordsMap
            boolean r2 = r2.containsKey(r10)
            if (r2 == 0) goto L7f
            java.util.Map<java.lang.Character, java.lang.String> r0 = cn.yunzhisheng.common.PinyinConverter.mUncommonWordsMap
        ...
        L93:
            byte[] r1 = cn.yunzhisheng.common.PinyinConverter.mUnit2PinyinIndex
            int r1 = getIntFrombytes(r1, r3, r8)
            r5[r2] = r1
            r1 = r5[r2]
            java.lang.String r1 = getPinYinbyIndex(r1)
            if (r0 != 0) goto La4
            r0 = r1
        La4:
            int r7 = r6.length()
